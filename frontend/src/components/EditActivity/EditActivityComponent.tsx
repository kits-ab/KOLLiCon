...
const EditActivity: React.FC<EditActivityProps> = ({
  ...
  const deleteTheThing = async (e: any) => {
    e.preventDefault();
    console.log(activityProp.id);
    setOpenAreYouSureModal(true);
  };

  const finalTermination = async (e: any) => {
    e.preventDefault();
    await axios.delete('http://localhost:8080/api/activity/delete/298');
    window.location.reload();
  };
    ...
    <GlobalBox>
      ...
      <EventsWrapper>
        <form onSubmit={handleSubmit}>
          <CancelButton
            style={{ position: 'relative', left: 320, bottom: 70 }}
            onClick={deleteTheThing}
          >
            remove
          </CancelButton>
          <StyledDiv>
      ...
      >
        <div style={{ position: 'relative', right: 20, padding: '20px' }}>
          <p style={{ position: 'relative', left: 20, marginBottom: '20px' }}>Are you sure fool?</p>
          <SaveButton
          ...
          <CancelButton onClick={() => setOpenAreYouSureModal(false)} color='primary'>
            NO
          </CancelButton>
        </div>
      </Dialog>
